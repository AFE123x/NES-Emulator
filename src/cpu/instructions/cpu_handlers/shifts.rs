...
impl Cpu {
    ...
    fn lsr_accumulator_helper(&mut self) {
        self.flags.set(Flags::Carry, self.a & 0x01 != 0); // Set carry flag to bit 0
        self.a >>= 1;
        self.flags.set(Flags::Zero, self.a == 0);
        self.flags.set(Flags::Negative, false); // Always clear after LSR
    }
    ...
    /// N    Negative Flag Always cleared (as bit 7 is always 0 after shift)
    pub fn lsr(&mut self) {
        if self.opcode == 0x4A {
    ...
    fn ror_accumulator(&mut self) {
        ...
        self.flags.set(Flags::Carry, self.a & 0x01 != 0);
        self.a >>= 1;
        self.a |= mask;
        self.flags.set(Flags::Zero, self.a == 0);
        self.flags.set(Flags::Negative, self.a & 0x80 != 0);
